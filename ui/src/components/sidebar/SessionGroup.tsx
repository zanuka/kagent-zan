...
const SessionGroup = ({ title, sessions, onViewRun, onDeleteRun }: SessionGroupProps) => (
  <div>
      ...
      {sessions.map((sessionWithRuns) => (
        <div key={sessionWithRuns.session.id} className="space-y-1">
          ...
          {sessionWithRuns.runs.map((run) => (
            <RunItem key={run.id} sessionId={sessionWithRuns.session.id!} run={run} onClick={onViewRun} onDelete={onDeleteRun} />
          ))}
        </div>
      ))}
    </div>
