        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                html_content = await page.evaluate(
                    '() => document.querySelector("main") ? document.querySelector("main").innerHTML : document.body.innerHTML'
                )
                ...
                clean_html = bleach.clean(article, tags=allowed_tags, attributes=allowed_attrs, strip=True)

                # Convert to markdown
                markdown = markdown2.markdown(clean_html, extras=["fenced-code-blocks", "tables"])
            ...
            while queue:
                ...
                try:
                    async with session.get(url) as response:
                        ...
                        if any(content_hash == existing_hash for existing_hash in self.page_contents.values()):
                        ...
                        output_file = os.path.join(
                            output_dir, f"{os.path.splitext(os.path.basename(relative_path))[0]}.md"
                        )
                        ...
                        for link in soup.find_all("a", href=True):
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    and crawl_path in full_url
                                    and normalize_url(full_url) not in self.visited_urls
                                ):
